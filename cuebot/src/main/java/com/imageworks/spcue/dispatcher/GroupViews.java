 ...
 * with a prefix minimum of memory need. GPU needs are walked, only for the candidates or hosts
 * that have them.
 */
final class GroupViews {

    /**
     * The idle hosts by idle cores descending, with the prefix maximum of idle memory. Built in
     * O(I log I); a CPU candidate's fit question costs O(log I).
     */
    static final class IdleView {
        final Scheduler.BookableHost[] byCores;
        final long[] prefixMaxMem;
        final long idleCoresSum;
        final List<Scheduler.BookableHost> reserved = new ArrayList<>();

        IdleView(List<Scheduler.BookableHost> hosts) {
            byCores = hosts.toArray(new Scheduler.BookableHost[0]);
            Arrays.sort(byCores, Comparator.comparingInt((Scheduler.BookableHost h) -> -h.coresIdle));
        ...
        /** How many hosts have at least {@code coresMin} idle cores: the length of the fitting prefix. */
    ...
    static final class NeedView {
        final Scheduler.LayerCandidate[] byCores;
        final long[] prefixMinMem;
        final List<Scheduler.LayerCandidate> gpuNeeders = new ArrayList<>();

        NeedView(List<Scheduler.LayerCandidate> candidates) {
            List<Scheduler.LayerCandidate> cpu = new ArrayList<>();
            ...
            byCores = cpu.toArray(new Scheduler.LayerCandidate[0]);
            Arrays.sort(byCores, Comparator.comparingInt((Scheduler.LayerCandidate c) -> c.layerCoresMin));
        ...
        boolean sellable(Scheduler.BookableHost h) {
            int k = servedPrefix(h.coresIdle);
            if (k > 0 && prefixMinMem[k - 1] <= h.memIdle)
                return true;
            for (Scheduler.LayerCandidate c : gpuNeeders) {
