...
/**
 * The draw of a group's placement slots: a priority-weighted lottery among the candidates of
 * the lowest-tier show on the allocation, with removal of the candidates that leave the draw.
 * The candidates live in one bucket per show, since the tier is a property of the show on this
 ...
 * O(shows + log C). A draw maps one integer in [0, sum of the tied weights) to a candidate
 * through bucket order, then member order.
 */
final class SlotDraw {
     ...
     * Bound: O(C).
     */
    SlotDraw(List<Scheduler.LayerCandidate> candidates) {
     ...
     * Bound: O(shows + log C).
     */
    Scheduler.LayerCandidate next(double u) {
        double low = Double.POSITIVE_INFINITY;
        for (Bucket b : buckets) {
            if (b.live == 0)
                continue;
            b.tier = Scheduler.showTier(b.members.get(0));
     ...
     */
    void remove(Scheduler.LayerCandidate c) {
        buckets.get(c.drawShow).remove(c.drawIx, Scheduler.lotteryWeight(c));
        live--;
    }
    ...
    private static final class Bucket {
        final int ix;
        final List<Scheduler.LayerCandidate> members = new ArrayList<>();
        long[] tree;
        long weight;
        int live;
        double tier;
        ...
        void seal() {
            ...
            tree = new long[n + 1];
            for (int i = 0; i < n; i++) {
                Scheduler.LayerCandidate c = members.get(i);
                c.drawShow = ix;
                c.drawIx = i;
                long w = Scheduler.lotteryWeight(c);
                tree[i + 1] = w;
                weight += w;
            }
            for (int i = 1; i <= n; i++) {
        ...
        Scheduler.LayerCandidate select(long r) {
            int n = members.size();
            int pos = 0;
