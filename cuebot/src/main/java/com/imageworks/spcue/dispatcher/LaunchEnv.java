 ...
 * environment is a property of the job and of the layer, not of the frame, so a batch keeps what
 * it has read; the maps are concurrent because the launch pool runs the batch on several threads.
 * The legacy per-frame paths hand in a fresh memo, so they read as before.
 */
final class LaunchEnv {
    ...
    Map<String, String> layer(DispatchFrame frame, LayerDao layerDao) {
        return byLayer.computeIfAbsent(frame.getLayerId(), k -> layerDao.getLayerEnvironment(frame));
    }
}
