...
public class DispatchSupportService implements DispatchSupport {
    ...
    public RunFrame prepareRqdRunFrame(VirtualProc proc, DispatchFrame frame, LaunchEnv env) {
        ...
        RunFrame.Builder builder = RunFrame.newBuilder().setShot(frame.shot).setShow(frame.show)
                ...
                .putAllEnvironment(env.job(frame, jobDao))
     ...
     * then files show, job and layer counters through the DAOs' batch methods: one JDBC round
     * trip per statement for the whole scoop instead of four to six per frame. A frame whose
     * usage read fails is skipped with a log line, exactly like the per-frame form.
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
