...
-- Now checks layer-level activation before frame-level dependencies
...
for _, frame_id in ipairs(frame_ids) do
    ...
    if redis.call('EXISTS', booking_key) == 0 then
        ...
        if frame.state and frame.cores_min and frame.memory_min then
            ...
            if frame.state == 'WAITING' and frame.job_active == 'true' then
                
                -- NEW: Check if layer is active
                local layer_active = true
                if frame.layer_id then
                    local layer_key = 'layer:' .. frame.layer_id
                    local layer_active_flag = redis.call('HGET', layer_key, 'active')
                    local layer_enabled = redis.call('HGET', layer_key, 'enabled')
                    
                    -- Layer must be both enabled and active (dependencies satisfied)
                    if layer_enabled ~= 'true' or layer_active_flag ~= 'true' then
                        layer_active = false
                    end
                end
                    ...
                    if cores_min <= available_cores and 
                       ...
                       gpu_min <= available_gpus then
                        ...
                        if service_match and show_active and not_paused then
                            -- Check frame-specific dependencies (if any)
                            ...
                            if depends_on_count > 0 then
                                -- Has frame-specific dependencies, need to check them
...
-- Log layer blocking stats
local blocked_by_layer = checked - #results
if blocked_by_layer > 0 then
    redis.call('HINCRBY', 'stats:dispatcher', 'frames_blocked_by_layer', blocked_by_layer)
end
