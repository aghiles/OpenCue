 ...
 * lowest-tier shows is picked as many times as its weight, and no other candidate is picked.
 * That is the invariant the linear walk enforced, compared here unit by unit rather than by a
 * sample. A second test pins the picks on a fixed farm, and a third checks that removal and a
 * moving tier hand the slots on as the walk did. The show cells stand in for the tick-wide
 * counters: one cell per show, shared by its candidates, as bindCells binds them.
 */
public class SlotDrawTests {

    private static Scheduler.LayerCandidate candidate(Map<String, Scheduler.Cell> cells,
            String layer, String show, int size, int coresInUse, int priority, int waiting) {
        ...
        c.showCell = cells.computeIfAbsent(c.showKey, k -> {
            Scheduler.Cell x = new Scheduler.Cell();
            x.used = coresInUse;
            return x;
        });
        c.priority = priority;
    ...
    private static Map<String, Long> expectedUnits(List<Scheduler.LayerCandidate> in) {
        double low = Double.POSITIVE_INFINITY;
        for (Scheduler.LayerCandidate c : in)
            low = Math.min(low, Scheduler.showTier(c));
        Map<String, Long> units = new HashMap<>();
        for (Scheduler.LayerCandidate c : in) {
            if (Scheduler.showTier(c) <= low)
                units.put(c.layerId, Scheduler.lotteryWeight(c));
        }
        return units;
    }
    ...
    private static Map<String, Long> drawnUnits(SlotDraw draw, long weightSum) {
        ...
        for (long r = 0; r < weightSum; r++) {
            Scheduler.LayerCandidate c = draw.next((r + 0.5) / weightSum);
    ...
    public void everyUnitLandsByWeightOnTheLowestTier() {
        ...
        for (int farm = 0; farm < 200; farm++) {
            ...
            List<Scheduler.LayerCandidate> all = new ArrayList<>();
            Map<String, Scheduler.Cell> cells = new HashMap<>();
            ...
            List<Scheduler.LayerCandidate> in = new ArrayList<>();
            for (Scheduler.LayerCandidate c : all)
            ...
            while (!in.isEmpty()) {
                ...
                Scheduler.LayerCandidate gone = in.remove(rnd.nextInt(in.size()));
                draw.remove(gone);
    ...
    public void pinnedPicksOnAFixedFarm() {
        ...
        Scheduler.LayerCandidate a = candidate(cells, "a", "showA", 100, 150, 500, 5);
        Scheduler.LayerCandidate b1 = candidate(cells, "b1", "showB", 100, 40, 1, 5);
        Scheduler.LayerCandidate b2 = candidate(cells, "b2", "showB", 100, 40, 50, 5);
        Scheduler.LayerCandidate b3 = candidate(cells, "b3", "showB", 100, 40, 100, 5);
        Scheduler.LayerCandidate c = candidate(cells, "c", "showC", 0, 0, 900, 5);
        List<Scheduler.LayerCandidate> all = List.of(a, b1, b2, b3, c);
        SlotDraw draw = new SlotDraw(all);
    ...
    public void aShowThatFillsPastItsSizeYieldsTheSlot() {
        Map<String, Scheduler.Cell> cells = new HashMap<>();
        Scheduler.LayerCandidate a = candidate(cells, "a", "showA", 100, 50, 10, 5);
        Scheduler.LayerCandidate b = candidate(cells, "b", "showB", 100, 80, 10, 5);
    ...
    public void candidatesWithoutFramesNeverEnterTheDraw() {
        Map<String, Scheduler.Cell> cells = new HashMap<>();
        Scheduler.LayerCandidate a = candidate(cells, "a", "showA", 100, 50, 10, 0);
        Scheduler.LayerCandidate b = candidate(cells, "b", "showA", 100, 50, 10, 3);
        SlotDraw draw = new SlotDraw(List.of(a, b));
