...
/**
 * One frame window per layer delivers to every (host, layer) plan the rows its own slice read
 * would have returned: the slices of a layer tile [0, planned total) without a gap or an overlap,
 * so the window read at that total covers each of them, and a ranking shorter than the plan clips
 * both the same way. Random plans, rankings shorter and longer than the plan.
 */
    ...
    public void layerWindowCutsToTheSameSlices() {
        ...
        for (int tick = 0; tick < 300; tick++) {
            ...
            for (int i = 0; i < rnd.nextInt(30); i++) {
                Scheduler.BookableHost h = hosts.get(rnd.nextInt(hosts.size()));
            ...
            for (String layer : layers) {
                ...
                for (Scheduler.BookableHost h : hosts) {
                    ...
                    assertEquals(bySliceRead, Scheduler.sliceOf(window, slice));
