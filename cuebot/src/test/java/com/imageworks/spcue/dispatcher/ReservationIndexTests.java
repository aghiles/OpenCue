 ...
 * Reservation names it, through any sequence of reserves, releases and sweeps. The targets a
 * layer claims are the free fitting hosts with the fewest running procs, in group order among
 * equals, exactly the hosts a repeated argmin with exclusion picks.
 */
public class ReservationIndexTests {
    ...
    public void theIndexAgreesWithAScanOfTheMap() {
        ...
        for (int step = 0; step < 2000; step++) {
            ...
            if (op < 6) {
                ...
                s.reserve(h, new Scheduler.Reservation(l, 100, 800));
                layerOf.put(h, l);
            } else if (op < 9) {
                String h = hosts[rnd.nextInt(hosts.length)];
                s.release(h);
    ...
    private static Scheduler.BookableHost argmin(Scheduler.LayerCandidate c,
            List<Scheduler.BookableHost> hosts, Set<String> taken) {
        Scheduler.BookableHost best = null;
        int bestProcs = Integer.MAX_VALUE;
        for (Scheduler.BookableHost h : hosts) {
            if (!Scheduler.hostCanEverFit(c, h) || taken.contains(h.hostId))
                continue;
    ...
    public void targetsMatchTheRepeatedArgmin() {
        ...
        for (int trial = 0; trial < 300; trial++) {
            ...
            for (Scheduler.BookableHost h : hosts) {
                if (rnd.nextInt(4) == 0) {
                    s.reserve(h.hostId, new Scheduler.Reservation("other", 100, 800));
                    taken.add(h.hostId);
                }
            }
            Scheduler.LayerCandidate c = Fix.candidate("wide", 800 * (1 + rnd.nextInt(4)),
                    Fix.GB, 0, 0, 0);
            int want = rnd.nextInt(8);
            List<Scheduler.BookableHost> got = s.reservationTargets(c, hosts, want);
            List<Scheduler.BookableHost> expected = new ArrayList<>();
            Set<String> excluded = new HashSet<>(taken);
            for (int i = 0; i < want; i++) {
                Scheduler.BookableHost t = argmin(c, hosts, excluded);
                if (t == null)
                    break;
                expected.add(t);
        ...
        assertNull(argmin(Fix.candidate("x", 800, Fix.GB, 0, 0, 0), new ArrayList<>(),
                new HashSet<>()));
        assertEquals(0, s.reservationTargets(Fix.candidate("x", 800, Fix.GB, 0, 0, 0),
                new ArrayList<>(), 3).size());
    }
}
