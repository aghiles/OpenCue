...
/**
 * Hosts and candidates for the scheduler's unit tests: explicit shapes, and random groups whose
 * hosts repeat a few states and carry GPUs, running layers, plans, warmth and reservations now and
 * then, so a shadow test meets every class of host the scan does.
 */
final class Fix {
    ...
    static Scheduler.BookableHost host(String id, int cores, long mem, int gpus, long gpuMem) {
    ...
    static Scheduler.BookableHost host(String id, int cores, int running) {
        Scheduler.BookableHost h = host(id, cores, 64 * GB, 0, 0);
        h.runningProcs = running;
        return h;
    }

    /** A candidate of the given needs with waiting frames, its caps open and its cells fresh. */
    static Scheduler.LayerCandidate candidate(String layerId, int cores, long mem, int gpus,
        ...
        c.rssProven = true;
        c.jobCell = new Scheduler.Cell();
        c.showCell = new Scheduler.Cell();
        return c;
    }

    /** A random host: three hardware classes, a few idle levels so classes form, one with GPUs. */
    static Scheduler.BookableHost randomHost(Random rnd, int ix) {
        int cls = rnd.nextInt(3);
        Scheduler.BookableHost h = host("h" + ix, 800 * (1 + cls), GB * 32 * (1 + cls),
        ...
        if (rnd.nextInt(8) == 0) {
            h.reservation = new Scheduler.Reservation(LAYERS[rnd.nextInt(LAYERS.length)], 100, 400);
    ...
    static Scheduler.LayerCandidate randomCandidate(Random rnd, int ix) {
        boolean gpu = rnd.nextInt(4) == 0;
        Scheduler.LayerCandidate c = candidate(LAYERS[ix % LAYERS.length],
                100 * (1 + rnd.nextInt(8)), GB * (1 + rnd.nextInt(16)), gpu ? 1 : 0,
                gpu ? GB * 4 : 0, rnd.nextInt(5));
        c.jobId = "j" + ix;
        c.rssProven = rnd.nextInt(4) != 0;
        c.clockTimeHighSec = rnd.nextBoolean() ? 0 : rnd.nextInt(7200);
        if (rnd.nextInt(3) == 0)
            c.licenses = Arrays.asList("lic");
        return c;
    }
}
