...
type (jaime/ram/elk), running-frame size mix, throughput, and the overbooking
...
print(f"{'t':>4} {'live%':>6} {'use%':>6} {'jaime%':>7} {'ram%':>6} {'elk%':>6} "
...
while time.time()-t0 < DURATION:
    ...
    # per host type util
    pt={"jaime(128c)":0.0,"ram(32c)":0.0,"elk(16c)":0.0}
    for row in q("""SELECT CASE WHEN h.int_cores>=12800 THEN 'jaime(128c)'
                              WHEN h.int_cores>=3200 THEN 'ram(32c)' ELSE 'elk(16c)' END t,
    ...
    el=time.time()-t0
    series.append((el,util,pt["jaime(128c)"],pt["ram(32c)"],pt["elk(16c)"],run,wait,done,busy,liveutil,zomb,usefulutil))
    print(f"{el:4.0f} {liveutil:6.1f} {usefulutil:6.1f} {pt['jaime(128c)']:7.1f} {pt['ram(32c)']:6.1f} "
          f"{pt['elk(16c)']:6.1f} {int(run):6d} {int(wait):7d} {int(done):7d} "
...
print(f"  at honest peak: jaime {peak[2]:.0f}%  ram {peak[3]:.0f}%  elk {peak[4]:.0f}%  "
      f"busyHosts {int(peak[8])}/1553")
jmax=max(s[2] for s in series); rmax=max(s[3] for s in series); emax=max(s[4] for s in series)
print(f"per-type peak util: jaime {jmax:.0f}%  ram {rmax:.0f}%  elk {emax:.0f}%")
