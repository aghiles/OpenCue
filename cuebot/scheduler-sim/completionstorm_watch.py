...
The invariant under test: post-completion work never runs on the planner
...
(postQ on the scheduler stat line) MUST fill; what happens next is the
verdict. If the tick inflates, the overflow ran the filing on the planner
...
Reads the farm through psql (procs, done count) and the scheduler's own
...
def main():
    ...
    if drops > 0:
        ...
        print(f"FAIL: avgTick peaked at {tick_pk}ms (> {TICK_MAX_MS}ms). The "
              f"full post-op queue ran its filing on the planner thread: tick "
