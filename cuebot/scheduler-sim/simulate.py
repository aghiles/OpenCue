...
def teardown():
    ...
    for pat in ["feed.py", "inject_big.py", "strand_watch.py", "gen_jobs.py",
                ...
                "rqd_report.py", "fake_rqd.py", "rqd_complete.py",
                "db_sampler.py", "util_sampler.py"]:
        pkill(pat)
    pkill("cue-scheduler")           # Rust scheduler (--mode rust)
...
def generate_graphs(graph_dir, sampler_procs, tag="run"):
    """Stop the samplers and render the util% and DB-load PNGs. Best-effort: a
    plotting failure warns but never fails the run."""
    ...
    r = sh([VENV_PY, "analysis/plot_run.py", graph_dir, tag], cwd=FARM, timeout=120)
