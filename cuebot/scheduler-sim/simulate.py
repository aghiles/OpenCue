...
def run_verify():
    ...
    scenarios = [
        ...
        ("SHOWTIER", ["--hosts", "3,4,10", "--showtier-test", str(max(D, 180))],
         {"SIM_DUR_LONG_S": "90"}),
        # SLICE: the planner sizes a slice at frame_query_max and charges the
        ...
        # to ONE background worker through a queue, and the planner must never
        ...
        # purpose (the planner commits once per layer per tick, so layers set
        ...
        # caller-runs overflow files on the planner and the tick trebles.
        ...
        ("COMPLETIONSTORM", ["--hosts", "0,0,80", "--cuebots", "1",
         ...
         {"SIM_DUR_LONG_S": "1", "SIM_STORM_JOBS": "400",
          ...
          "SCHEDULER_POST_COMPLETE_QUEUE_SIZE": "300",
...
def main():
    ...
    ap.add_argument("--slice-test", type=int, default=0, metavar="SECS",
                    help="SLICE test: one wide one-core layer on three large "
                         ...
                         "large host is the slice the planner accounted "
    ...
    if args.priority_starve:
        ...
        log(f"watching SLICE (a slice delivers what the planner accounted) "
