"""SLICE watcher: a slice delivers what the planner accounted.
...
tick. The planner sizes a first slice on an idle 128-core host at
...
FAIL      : the disease. A first delivery below SLICE on a host that had the
            room: the plan read cut the slice the planner accounted, so the
...
def main():
    print(f"watching SLICE for {DURATION}s: the first delivery on every large host must be "
          f"at least {SLICE} frames (frame_query_max), the slice the planner accounted.\n",
          flush=True)
    t0 = time.time()
    first = {}
    last = {}
    ...
    if short:
        print(f"FAIL: {len(short)} of {len(first)} large hosts got a first slice below "
              ...
              + "). The plan read cut the slice the planner accounted.", flush=True)
    else:
        print(f"PASS: every large host's first slice delivered at least {SLICE} frames, "
              f"the size the planner accounted.", flush=True)
